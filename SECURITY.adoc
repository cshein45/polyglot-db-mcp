...
* Query parameters use parameterized queries where supported
* **Important**: Table/column identifiers are NOT sanitized in some adapters
** SQL-based adapters (SQLite, MariaDB, PostgreSQL) accept raw identifiers
** This is by design for MCP use cases (trusted AI clients)
** For untrusted input, use the query tools with full parameterization
